...
/**
 * A stream wrapper for temporary data.
 *
 * Use it like memory://<identifier>
 *
 * @version $Id: MemoryStreamWrapper.php 3611 2010-01-13 15:24:42Z k-fish $
 * @license http://www.gnu.org/licenses/lgpl.html GNU Lesser General Public License, version 3 or later
 * @scope prototype
 */
class MemoryStreamWrapper implements \F3\FLOW3\Resource\Streams\StreamWrapperInterface {
	...
	public function isAtEof() {
        return $this->position >= strlen($GLOBALS['MemoryStreamWrapper'][$this->varname]);
	}
	...
	public function open($path, $mode, $options, &$openedPathAndFileName) {
		...
		switch ($mode[0]) {
			case 'w':
				$GLOBALS['MemoryStreamWrapper'][$this->varname] = '';
			break;
			case 'a':
				if (!isset($GLOBALS['MemoryStreamWrapper'][$this->varname])) {
					$GLOBALS['MemoryStreamWrapper'][$this->varname] = '';
				}
				$this->position = strlen($GLOBALS['MemoryStreamWrapper'][$this->varname]);
			break;
			case 'r':
				if (!isset($GLOBALS['MemoryStreamWrapper'][$this->varname])) {
					return FALSE;
	...
	public function read($count) {
        $data = substr($GLOBALS['MemoryStreamWrapper'][$this->varname], $this->position, $count);
	...
	public function seek($offset, $whence = SEEK_SET) {
        switch ($whence) {
            case SEEK_SET:
                if ($offset < strlen($GLOBALS['MemoryStreamWrapper'][$this->varname]) && $offset >= 0) {
            ...
            case SEEK_END:
                if (strlen($GLOBALS['MemoryStreamWrapper'][$this->varname]) + $offset >= 0) {
                     $this->position = strlen($GLOBALS['MemoryStreamWrapper'][$this->varname]) + $offset;
                     return TRUE;
                } else {
                     return FALSE;
                }
	...
	public function write($data) {
        $left = substr($GLOBALS['MemoryStreamWrapper'][$this->varname], 0, $this->position);
        $right = substr($GLOBALS['MemoryStreamWrapper'][$this->varname], $this->position + strlen($data));
        $GLOBALS['MemoryStreamWrapper'][$this->varname] = $left . $data . $right;
        $this->position += strlen($data);
        return strlen($data);
	}
